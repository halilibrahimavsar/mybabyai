"""
CodeMind — Yerel Eğitim Testi (GTX 1650 / 4 GB VRAM)

Strateji:
  • Model   : CodeMind-125M (sıfırdan / eğitilmemiş)
  • Tip      : LoRA (sadece ~2-5M parametre eğitilir)
  • Batch    : 1 + gradient_accumulation=8 → efektif batch=8
  • Seq len  : 128 token (kısa; VRAM tasarrufu)
  • Veri     : 200 örnek dummy conversation (HF indirme yok)
  • Epoch    : 1 (hızlı sanity-check)
  • Hedef    : Hata olmadan tamamlanırsa ülke testi geçti
...
Sonraki adım (başarılı olursa):
  → 350M-MoE + gerçek veri → Kaggle T4x2 / A100 üzerinde tam eğitim
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. Konfigürasyon — GTX 1650 için ultra-konservatif
# ─────────────────────────────────────────────────────────────────────────────
def build_config() -> Config:
    cfg = Config()
    # Model
    cfg.set("model.name", "CodeMind-125M")
    cfg.set("model.device", "cuda")
    cfg.set("model.load_in_4bit", False)   # QLoRA 4-bit kapalı: GTX 1650 bnb desteği kırık
    # LoRA — sadece q_proj + v_proj (minimum VRAM)
    cfg.set("model.lora.r", 8)             # küçük rank → az parametre
    cfg.set("model.lora.lora_alpha", 16)
    cfg.set("model.lora.lora_dropout", 0.05)
    cfg.set("model.lora.target_modules", ["q_proj", "v_proj"])
    # Eğitim
    cfg.set("training.output_dir", "models/local_test")
    cfg.set("training.per_device_train_batch_size", 1)
    cfg.set("training.gradient_accumulation_steps", 8)   # efektif batch = 8
    cfg.set("training.gradient_checkpointing", True)      # VRAM yarı yarıya düşer
    cfg.set("training.max_length", 128)                   # en kritik VRAM tasarrufu
    ...
    cfg.set("training.pack_sequences", False)  # küçük veri seti, pack kapalı
...
def main():
    logger.info("=" * 60)
    logger.info("  CodeMind Yerel Eğitim Testi — GTX 1650 / 4 GB VRAM")
    logger.info("=" * 60)
    ...
    logger.info("\n[2/5] CodeMind-125M sıfır model oluşturuluyor...")
    model_manager = ModelManager(config)
    model_manager.load_fresh_model(size="125M")
    ...
    logger.info("  • Batch: 1 (grad_acc=8 → efektif 8)")
    logger.info("  • Seq length: 128 token")
    ...
    try:
        ...
        logger.info("\n[5/5] Test Sonuçları:")
        logger.info("=" * 60)
        logger.info("✅ YERELv EĞITIM TESTİ BAŞARILI!")
        logger.info(f"  Loss         : {metrics.get('train_loss', 'N/A'):.4f}")
        logger.info(f"  Toplam adım  : {metrics.get('train_steps_per_second', 'N/A')}")
        logger.info(f"  Süre (sn)    : {metrics.get('train_runtime', 'N/A'):.1f}")
        logger.info("=" * 60)
        logger.info("")
        logger.info("Bir sonraki adım:")
        logger.info("  → 350M-MoE modelini Kaggle / Google Colab A100'de eğit")
        logger.info("  → cloud_train.ipynb kullan, model_size='350M-MoE' seç")
        logger.info("  → Veri: UltraChat+SlimOrca+Türkçe+Kod (~38K örnek başlangıç)")

    except torch.cuda.OutOfMemoryError:
        logger.error("\n❌ CUDA Bellek Hatası (OOM)!")
        logger.error("Çözüm önerileri:")
        logger.error("  1. training.max_length: 64'e düşür (şu an 128)")
        logger.error("  2. training.gradient_accumulation_steps: 16'ya çıkar")
        logger.error("  3. Başka uygulamaları kapat, GPU boşalt")
        logger.error("  4. CPU modunda çalıştır (cok yavaş ama OOM olmaz):")
        logger.error("     config.set('model.device', 'cpu')")
        sys.exit(1)
